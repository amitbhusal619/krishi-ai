...
export default function FarmerProfilePage() {
  ...
  const displayName = [user?.first_name, user?.last_name].filter(Boolean).join(" ").trim() || user?.username || "Farmer";
      ...
      <Card className="max-w-2xl">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-dark/50">{user?.role ? `${user.role.charAt(0).toUpperCase()}${user.role.slice(1)}` : "Farmer"}</p>
          </div>
        </div>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input value={user?.role ? `${user.role.charAt(0).toUpperCase()}${user.role.slice(1)}` : "Farmer"} readOnly className="leaf-shape-sm mt-1 w-full border border-dark/10 bg-white/80 px-4 py-3 text-sm outline-none" />
          </div>
        </div>
